...
public class MovieDetailViewModel extends AndroidViewModel {
    ...
    public void loadReviews(int id){
        Disposable disposable = ApiFactory.apiServise.loadReviews(id)
                ...
                .map(new Function<ReviewResponce, List<Review>>() {
                    @Override
                    public List<Review> apply(ReviewResponce reviewResponce) throws Throwable {
                        return reviewResponce.getReviews();
                    }
                })
    ...
    public void loadTrailers(int id){
        Disposable disposable = ApiFactory.apiServise.loadTrailers(id)
                .subscribeOn(Schedulers.io()) //переключение потока
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public void removeMovie(int movieId){
        Disposable disposable = movieDao.remuveMovie(movieId)
                .subscribeOn(Schedulers.io())
                .subscribe();
        compositeDisposable.add(disposable);
    }
